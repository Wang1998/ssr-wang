...
==========================================================================
		           ...
		           欢迎访问 旺专用SSR搭建                               All Rights Reserved							                  	
==========================================================================';
echo "$CopyrightLogo";
# FILES  ******************************************************************
ServerLocation='master';
# VAR	******************************************************************	https://api.ip.la 这个接口查IP也比较好用
Model=$1;
Froms=$2;
...
echo -n "请输入数据库密码(自用默认密码)： "
read mysqlpass
if [ -z $mysqlpass ]
then
echo  "数据库密码：hw8781733"
mysqlpass=hw8781733
else
        echo "数据库密码：$mysqlpass"
        fi
        echo -n "请输入后台登陆账号，请使用邮箱方式(默认774562524@qq.com)： "
read gly
if [ -z $gly ]
then
echo  "后台账号：774562524@qq.com"
gly=774562524@qq.com
else
        echo "后台账号：$gly"
        fi
        echo -n "请输入SS连接端口，(默认140)： "
read proxy
if [ -z $proxy ]
then
echo  "SS连接端口：140"
...
cd /var/www/html
wget https://github.com/Wang1998/ssr-wang/raw/master/SSR/ss-panel.zip >/dev/null 2>&1
#旧源 自己修改过的SSR  wget ${webs}${MirrorHost}/${ServerLocation}/ss-panel.zip >/dev/null 2>&1
